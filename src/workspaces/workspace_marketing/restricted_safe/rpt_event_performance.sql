...
campaign_members AS (
  SELECT
    ...
    map_person_territory.report_sales_segment                     AS report_person_segment, 
    -- necessary for the fixed calculation in Tableau for Attendance of records with no Account ID.
    {{ dbt_utils.generate_surrogate_key([
    'sfdc_campaign_member.campaign_id',
    'map_person_territory.report_geo',
    'map_person_territory.report_region',
    'map_person_territory.report_area',
    'map_person_territory.report_sales_segment'
    ]) }}                                                         AS pseudo_account_id,
    IFNULL(mart_crm_person.dim_crm_account_id, pseudo_account_id) AS person_account_id,
  ...
  LEFT JOIN map_person_territory
    ...
    AND mart_crm_person.dim_crm_account_id IS NULL 
...
account_summary AS (
  SELECT 
    ...
    report_person_geo,
    report_person_region,
    report_person_area,
    report_person_segment,
    true_event_date,
    campaign_name,
    open_pipeline_live,
    COUNT(DISTINCT
...
opportunity_campaign_snapshot_prep AS (
  SELECT
    ...
    CASE 
      WHEN opportunity_snapshot_base.pipeline_created_date >= account_summary.true_event_date AND 
      opportunity_snapshot_base.is_net_arr_pipeline_created 
      THEN opp_net_arr 
    END
...
unified_attribution_touchpoint_base AS (
  -- MIP touchpoints from snapshots (existing logic)
  SELECT 
    ...
    sfdc_bizible_attribution_touchpoint_snapshots_source.bizible_weight_custom_model / 100 AS mip_weight,
    0 AS is_mgp_opportunity, -- we get MGP snapshot below, 
    0 AS mgp_weight,  -- we get MGP snapshot below
    'MIP' AS attribution_type
  ...
  -- MGP touchpoints from live data
  SELECT 
    snapshot_dates.date_day AS touchpoint_snapshot_date,
    snapshot_dates.dim_campaign_id,
    snapshot_dates.event_snapshot_type,
    ...
    0 AS mip_weight,  -- We get MIP from snapshots, not here
    CASE 
      WHEN mart_crm_attribution_touchpoint.pipeline_created_date <= snapshot_dates.date_day
      THEN IFF(mart_crm_attribution_touchpoint.is_mgp_opportunity = TRUE, 1, 0)
      ELSE 0
      END AS is_mgp_opportunity, 
    CASE 
      WHEN mart_crm_attribution_touchpoint.pipeline_created_date <= snapshot_dates.date_day
      THEN mart_crm_attribution_touchpoint.gitlab_model_weight 
      ELSE 0
      END AS mgp_weight,
    'MGP' AS attribution_type
  FROM mart_crm_attribution_touchpoint
  INNER JOIN snapshot_dates 
    ON mart_crm_attribution_touchpoint.dim_campaign_id = snapshot_dates.dim_campaign_id
  WHERE 
    mart_crm_attribution_touchpoint.is_mgp_opportunity = TRUE
),

-- Consolidate weights for touchpoints that may have both MIP and MGP
consolidated_attribution_touchpoints AS (
  SELECT
    touchpoint_snapshot_date,
    dim_campaign_id,
    event_snapshot_type,
    dim_crm_touchpoint_id,
    dim_crm_opportunity_id,
    bizible_touchpoint_date,
    MAX(is_mgp_opportunity) AS is_mgp_opportunity,
    SUM(mip_weight) AS total_mip_weight,
    SUM(mgp_weight) AS total_mgp_weight
  FROM unified_attribution_touchpoint_base
...
combined_models AS (
  ...
  --IDs
    ...
    consolidated_attribution_touchpoints.dim_crm_touchpoint_id,
    consolidated_attribution_touchpoints.dim_campaign_id,
    --Dates
    ...
    consolidated_attribution_touchpoints.bizible_touchpoint_date,
    consolidated_attribution_touchpoints.touchpoint_snapshot_date,
    ...
    consolidated_attribution_touchpoints.event_snapshot_type,
    --Campaign Dimensions 
    campaigns.campaign_name,
    ...
    consolidated_attribution_touchpoints.is_mgp_opportunity,

    --Metrics
    opportunity_snapshot_base.opp_net_arr,
    -- Attributed amounts
    consolidated_attribution_touchpoints.total_mip_weight AS bizible_weight_custom_model,
    consolidated_attribution_touchpoints.total_mgp_weight AS gitlab_model_weight,
    consolidated_attribution_touchpoints.total_mip_weight * opportunity_snapshot_base.opp_net_arr AS influenced_net_arr,
    consolidated_attribution_touchpoints.total_mgp_weight * opportunity_snapshot_base.opp_net_arr AS gitlab_model_net_arr

  FROM consolidated_attribution_touchpoints
  LEFT JOIN opportunity_snapshot_base
    ON consolidated_attribution_touchpoints.dim_crm_opportunity_id = opportunity_snapshot_base.dim_crm_opportunity_id
      AND consolidated_attribution_touchpoints.touchpoint_snapshot_date = opportunity_snapshot_base.opportunity_snapshot_date
  LEFT JOIN campaigns
    ON consolidated_attribution_touchpoints.dim_campaign_id = campaigns.dim_campaign_id
...
aggregated_opportunity_influenced_performance AS (
  SELECT
    ...
    combined_models.is_mgp_opportunity,
    ...
    SUM(CASE WHEN is_net_arr_pipeline_created = 1 THEN gitlab_model_net_arr END) AS marketing_generated_pipeline
  FROM
    combined_models
...
final AS (
  ...
  --ACCOUNT FIELDS 
    ...
    IFNULL(mart_crm_account.parent_crm_account_geo, account_summary.report_person_geo)                          AS parent_crm_account_geo,
    IFNULL(mart_crm_account.parent_crm_account_region, account_summary.report_person_region)                    AS parent_crm_account_region,
    IFNULL(mart_crm_account.parent_crm_account_sales_segment, account_summary.report_person_segment)            AS parent_crm_account_sales_segment,
    IFNULL(mart_crm_account.parent_crm_account_area, account_summary.report_person_area)                        AS parent_crm_account_area,
  ...
  --Opportunity dimensions
    ...
    opportunity_snapshot_base.snapshot_is_net_arr_pipeline_created,
    opportunity_snapshot_base.snapshot_is_booked_net_arr,
    aggregated_opportunity_influenced_performance.is_mgp_opportunity,
  ...
  --Pipeline/Opp Metrics
    ...
    aggregated_opportunity_influenced_performance.marketing_generated_pipeline
