...
discount_prep_step_1 AS (
  ...
  SELECT
    ...
    CASE
      ...
      ELSE 'Growth'
    END                                                                                                        AS order_type,
    COALESCE(dim_crm_opportunity_deal.deal_path, 'Other')                                                      AS deal_path_name,
    COALESCE(sheetload_partner_discount.partner_margin, 
            (dim_crm_opportunity_partner.partner_margin_percentage / 100))                                     AS partner_margin
...
discount_prep_step_2 AS (
  ...
  SELECT
    ...
    CASE
      ...
      WHEN quantity_with_addon_update = 0
        THEN 'NE: Quantity=0'
      WHEN subscription_name LIKE '%UPLIFT RENEWAL AND TRUEUP' OR subscription_name = 'A-S00110043'
        THEN 'Deal in FY24-Q2 99.5% discount, deal in FY25-Q3 99.94% discount'
...
discount_prep_step_4 AS (
  ...
  SELECT
    ...
    ROUND(SUM(arr_calc), 2)                                               AS arr,
    ROUND(SUM(arr_incl_channel_margin), 2)                                AS arr_with_channel_margin,
