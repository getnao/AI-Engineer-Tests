...
final AS (
  SELECT
    ...
    issues.author_is_blocked_user,
    issues.author_is_team_member,
    issues.updated_at,
    issues.state                                                                                                                                                                       AS recorded_state,
    issues.weight,
  ...
  FROM issues
  INNER JOIN dates
    ON dates.date_actual BETWEEN DATE_TRUNC('day', issues.created_at) AND COALESCE(DATE_TRUNC('day', issues.closed_at), current_date)
