...
{% docs gitlab_dotcom_notes_xf %}

This model contains notes (comments) from across GitLab.com, with privacy protections for non-public content. It transforms the raw notes data by masking note content from non-public, non-internal namespaces to protect customer confidentiality. The model includes metadata about each note, its author, and relevant relationships, augmented with system note action types. This incremental model supports analysis of collaboration patterns, discussions, and user engagement with GitLab objects while respecting privacy concerns.

{% enddocs %}


{% docs gitlab_dotcom_milestones_xf %}

This model contains milestone data from GitLab.com, with privacy protections for non-internal milestone content. It transforms the raw milestone data by masking milestone titles and descriptions from non-internal namespaces to protect customer confidentiality. The model combines milestone information with project and namespace context, providing a comprehensive view of milestones across GitLab. This model supports analysis of planning processes, timeline adherence, and milestone usage patterns while respecting privacy concerns.
...
This model captures user activity events in the GitLab.com application, enhanced with project, subscription, and user context. It incrementally processes events data and joins it with projects, subscriptions, plans, and users information to provide a comprehensive view of user actions. The model includes subscription plan information at the time of the event, allowing for user activity analysis by subscription tier. It filters out events from blocked users and provides essential context for user behavior analysis across the GitLab platform.

{% enddocs %}


{% docs gitlab_dotcom_epic_notes_xf %}

This model contains notes (comments) associated with epics in GitLab.com, with privacy protections for non-public content. It transforms the raw notes data by masking note content from non-public, non-internal namespaces to protect customer confidentiality. The model includes metadata about each note, its author, and its relationship to the epic, with join paths to relevant dimensions. This incremental model supports analysis of collaboration patterns, epic discussions, and user engagement with epics while respecting privacy concerns.
