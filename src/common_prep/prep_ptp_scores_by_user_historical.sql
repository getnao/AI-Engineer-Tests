{% set columns = ["dim_marketing_contact_id", "score_group", "ptp_source", "last_score_date", "submodel", "valid_from", "valid_to"] %}
...
, combined_score_dates AS (
  ...
  SELECT
    ...
    */
    ...
    COALESCE(prep_ptpl_scores_by_user.submodel,
      LAG(prep_ptpl_scores_by_user.submodel, 1, NULL) IGNORE NULLS OVER (PARTITION BY dim_marketing_contact_id_combined ORDER BY score_date_combined))
                                                  AS latest_ptpl_submodel,
    ...
    CASE
      ...
      WHEN latest_ptpf = 5 AND DATEDIFF('DAY', latest_ptpf_score_date, score_date_combined) <= 45 -- Only use if score has been updated in last 45 days
        THEN 'Free'
      WHEN latest_ptpl = 5 AND DATEDIFF('DAY', latest_ptpl_score_date, score_date_combined) <= 45 AND latest_ptpl_submodel = 'no_oppty_model'-- Only use if score has been updated in last 45 days
        THEN 'Lead - No Opps'
      WHEN latest_ptpl = 5 AND DATEDIFF('DAY', latest_ptpl_score_date, score_date_combined) <= 45 AND latest_ptpl_submodel = 'oppty_model' -- Only use if score has been updated in last 45 days
        THEN 'Lead - Open Opps'
      WHEN latest_ptpf >= 4 AND DATEDIFF('DAY', latest_ptpf_score_date, score_date_combined) <= 45 -- Only use if score has been updated in last 45 days
        THEN 'Free'
       WHEN latest_ptpl >= 4 AND DATEDIFF('DAY', latest_ptpl_score_date, score_date_combined) <= 45 AND latest_ptpl_submodel = 'no_oppty_model'-- Only use if score has been updated in last 45 days
        THEN 'Lead - No Opps'
      WHEN latest_ptpl >= 4 AND DATEDIFF('DAY', latest_ptpl_score_date, score_date_combined) <= 45 AND latest_ptpl_submodel = 'oppty_model' -- Only use if score has been updated in last 45 days
        THEN 'Lead - Open Opps'
      ...
      WHEN prep_ptpf_scores_by_user.dim_marketing_contact_id IS NOT NULL
        THEN 'Free'
      WHEN prep_ptpl_scores_by_user.dim_marketing_contact_id IS NOT NULL AND latest_ptpl_submodel = 'no_oppty_model'
        THEN 'Lead - No Opps'
      WHEN prep_ptpl_scores_by_user.dim_marketing_contact_id IS NOT NULL AND latest_ptpl_submodel = 'oppty_model'
        THEN 'Lead - Open Opps'
    END AS ptp_source,
...
SELECT
  ...
  ptp_source,
  last_score_date,
  latest_ptpl_submodel AS submodel,
  valid_from,
  valid_to
FROM valid_to_from
