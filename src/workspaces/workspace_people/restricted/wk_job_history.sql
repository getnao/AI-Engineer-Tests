...
staff_hist AS (
  ...
  SELECT  
    *,
    LAG(effective_date,1,TO_DATE('2099-01-01', 'YYYY-MM-DD')) OVER (PARTITION BY employee_id ORDER BY effective_date DESC,date_time_initiated DESC) AS next_effective_date
...
key_talent AS (
  ...
  SELECT *,
    LAG(effective_date,1,TO_DATE('2099-01-01', 'YYYY-MM-DD')) OVER(PARTITION BY employee_id ORDER BY effective_date DESC, uploaded_at DESC) AS next_effective_date
...
performance_review AS (
  ...
  SELECT *,
    LAG(review_period_start_date,1,TO_DATE('2099-01-01', 'YYYY-MM-DD')) OVER(PARTITION BY employee_id ORDER BY review_period_start_date DESC, review_period_end_date DESC) AS review_end_date
...
eda AS (
  ...
  SELECT    
    ...
    IFF(eda_stage.job_title = 'Code2College Intern'
      AND date_actual < '2022-06-16','Intern (Trainee)', staff_hist.employee_type_current)                          AS employee_type,
    cost_center,
    ...
    COALESCE(key_talent.is_key_talent, FALSE)                                                                       AS is_key_talent,
    performance_review.performance_rating,
    performance_review.growth_potential_rating                                                                      AS potential_rating        
  FROM eda_stage
  ...
  LEFT JOIN key_talent
    ON eda_stage.employee_id = key_talent.employee_id
      AND eda_stage.date_actual >= key_talent.effective_date
      AND eda_stage.date_actual < key_talent.next_effective_date
  LEFT JOIN performance_review
    ON eda_stage.employee_id = performance_review.employee_id
      AND eda_stage.date_actual >= review_period_start_date
      AND eda_stage.date_actual < review_end_date      
...
pr AS (
  ...
  SELECT 
    ...
    is_key_talent               AS pr_is_key_talent,
    performance_rating          AS pr_performance_rating,
    potential_rating            AS pr_potential_rating
  FROM eda 
...
hist_stage AS (
  ...
  SELECT    
    ...
    CASE
      ...
      WHEN COALESCE(is_key_talent, FALSE) != COALESCE(pr_is_key_talent, FALSE) THEN 1
      WHEN COALESCE(performance_rating,'1') != COALESCE(pr_performance_rating,'1') THEN 1
      WHEN COALESCE(potential_rating,'1') != COALESCE(pr_potential_rating,'1') THEN 1      
      ELSE 0
    END AS filter
  FROM eda cur
  LEFT JOIN pr
...
SELECT   
  ...
  CASE
    ...
    WHEN COALESCE(hist_stage.is_key_talent, FALSE) != COALESCE(pr_is_key_talent, FALSE) THEN 'Talent Change'
    WHEN COALESCE(hist_stage.performance_rating, '1') != COALESCE(pr_performance_rating, '1') THEN 'Talent Change'
    WHEN COALESCE(hist_stage.potential_rating, '1') != COALESCE(pr_potential_rating, '1') THEN 'Talent Change'    
    ELSE NULL
