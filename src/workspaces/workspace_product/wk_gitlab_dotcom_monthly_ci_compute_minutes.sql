...
WITH pipeline_activity AS (
  ...
  SELECT
    ...
    dim_project_id, 
    dim_user_id,
    mart_ci_runner_activity.dim_ci_runner_id,
    CASE 
      WHEN plan_name_modified LIKE '%trial%' 
        THEN 'Trial' --plan_name_modified maps to current plan names
      WHEN plan_name_modified = 'ultimate' 
        THEN 'Ultimate'
      WHEN plan_name_modified = 'premium' 
        THEN 'Premium'
      WHEN plan_name_modified = 'default' 
        THEN 'Free'
      ELSE plan_title
    END                                                             AS plan_title,
    CASE
    ...
    ci_pipeline_source,
    executor_type,
    CASE WHEN DATEDIFF('day', contacted_at, current_date) >= 7
      THEN 'stale'
      WHEN contacted_at IS NULL
      THEN 'never_contacted'
      WHEN DATEDIFF('day', contacted_at, current_date) <= 7
      THEN 'active'
      ELSE 'inactive'
    END AS runner_status,
    ci_runner_machine_type,
    cost_factor,
    COUNT(DISTINCT mart_ci_runner_activity.dim_ci_pipeline_id)       AS count_of_pipelines,
    SUM(ci_build_duration_in_s) / 60                                AS ci_build_minutes
  FROM {{ ref('mart_ci_runner_activity') }}
  INNER JOIN {{ ref ('dim_ci_runner') }}
    ON mart_ci_runner_activity.dim_ci_runner_id = dim_ci_runner.dim_ci_runner_id
  INNER JOIN {{ ref ('dim_namespace') }}
    ON mart_ci_runner_activity.dim_namespace_id = dim_namespace.dim_namespace_id
  INNER JOIN {{ ref('prep_gitlab_dotcom_plan') }}
    ON mart_ci_runner_activity.dim_plan_id = prep_gitlab_dotcom_plan.dim_plan_id
  WHERE ci_build_started_at >= '2020-01-01'
    AND ci_build_finished_at IS NOT NULL
    AND dim_namespace.namespace_is_internal = FALSE
    AND dim_namespace.namespace_creator_is_blocked = FALSE
  GROUP BY ALL
  ...
  SELECT 
   ...
   ) }}                                                       AS namespace_reporting_month_pk,
   pipeline_activity.reporting_month, 
   pipeline_activity.ultimate_parent_namespace_id, 
   plan_title,
   dim_project_id,
   ...
   CASE 
     WHEN cost_factor = 0 
       THEN ci_build_minutes
     ELSE ci_build_minutes * cost_factor
   END                                                         AS compute_minutes_with_cost_factor,
   CASE 
     WHEN plan_title = 'Free' 
       THEN 400
     WHEN plan_title = 'Premium' 
       THEN 10000
     WHEN plan_title = 'Ultimate' 
       THEN 50000
     ELSE 400
