...
case_output AS (
  ...
  LEFT JOIN prep_crm_case
    ON distinct_cases.account_id = prep_crm_case.dim_crm_account_id
      AND (
        distinct_cases.case_subject = prep_crm_case.subject
      OR (
        -- The distinct_cases subject equals the prep_crm_case subject without the advocate tag
        lower(distinct_cases.case_subject) = 
          TRIM(REGEXP_REPLACE(LOWER(prep_crm_case.subject), '\\s*#advocate(s)?\\s*$', ''))
        )
        )
