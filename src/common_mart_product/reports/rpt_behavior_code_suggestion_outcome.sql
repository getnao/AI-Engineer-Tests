...
WITH gitlab_ide_extension_events AS (
  ...
  ) --events that can be used to calculate suggestion outcome
...
suggestion_level AS (
  ...
  SELECT
    ...
    requested.resolution_strategies,
    requested.feature_enablement_type,
    ...
    COALESCE(requested.region, loaded.region, shown.region,
      accepted.region, rejected.region,
      cancelled.region)                                                                             AS region,
