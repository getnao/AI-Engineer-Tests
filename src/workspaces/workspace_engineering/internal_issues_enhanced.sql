...
dim_user AS (
  ...
  SELECT dim_user_id, is_bot, is_blocked_user, user_name
  FROM {{ ref('dim_user') }}
...
final AS (
  ...
  SELECT
    ...
    COALESCE(dim_user.is_bot,FALSE)                                                                                                                                                                                                                                                                     AS is_created_by_bot,
    COALESCE(dim_user.is_blocked_user,FALSE)                                                                                                                                                                                                                                                            AS author_is_blocked_user,
    dim_user.user_name                                                                                                                                                                                                                                                                                  AS author_user_name,
    IFF(members.user_id IS NOT NULL, TRUE, FALSE)                                                                                                                                                                                                                                                       AS author_is_team_member,
    ...
    CASE
      WHEN full_group_path like '%private - masked%' THEN null
  ...
  LEFT JOIN dim_user
    ON internal_issues.author_id = dim_user.dim_user_id
  ...
  LEFT JOIN {{ ref('map_team_member_user')}} AS members
    ON internal_issues.author_id = members.user_id
)
