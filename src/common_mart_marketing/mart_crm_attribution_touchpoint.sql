...
, joined AS (
    ...
    SELECT
      ...
      dim_crm_touchpoint.dim_crm_buyer_touchpoint_id,
      ...
      CASE
            WHEN dim_crm_touchpoint.bizible_touchpoint_date < dim_date_stage0.date_actual
              THEN 'Pre Opp Creation'
            WHEN dim_crm_touchpoint.bizible_touchpoint_date >= dim_date_stage0.date_actual 
              AND (dim_crm_touchpoint.bizible_touchpoint_date < dim_date_stage1.date_actual 
                  OR dim_date_stage1.date_actual IS NULL)
              THEN 'Stage 0'
            WHEN dim_crm_touchpoint.bizible_touchpoint_date >= dim_date_stage1.date_actual 
              AND (dim_crm_touchpoint.bizible_touchpoint_date < dim_date_stage2.date_actual 
                  OR dim_date_stage2.date_actual IS NULL)
              THEN 'Stage 1'
            WHEN dim_crm_touchpoint.bizible_touchpoint_date >= dim_date_stage2.date_actual 
              AND (dim_crm_touchpoint.bizible_touchpoint_date < dim_date_stage3.date_actual 
                  OR dim_date_stage3.date_actual IS NULL)
              THEN 'Stage 2'
            WHEN dim_crm_touchpoint.bizible_touchpoint_date >= dim_date_stage3.date_actual
              AND (dim_crm_touchpoint.bizible_touchpoint_date < dim_date_stage4.date_actual 
                  OR dim_date_stage4.date_actual IS NULL)
              THEN 'Stage 3'
            WHEN dim_crm_touchpoint.bizible_touchpoint_date >= dim_date_stage4.date_actual 
              AND (dim_crm_touchpoint.bizible_touchpoint_date < dim_date_stage5.date_actual 
                  OR dim_date_stage5.date_actual IS NULL)
              THEN 'Stage 4'
            WHEN dim_crm_touchpoint.bizible_touchpoint_date >= dim_date_stage5.date_actual 
              AND (dim_crm_touchpoint.bizible_touchpoint_date < dim_date_stage6.date_actual 
                  OR dim_date_stage6.date_actual IS NULL)
              THEN 'Stage 5'
            WHEN dim_crm_touchpoint.bizible_touchpoint_date >= dim_date_stage6.date_actual 
              AND ((dim_crm_touchpoint.bizible_touchpoint_date < dim_date_won.date_actual 
                    OR dim_crm_touchpoint.bizible_touchpoint_date < dim_date_lost.date_actual) 
                  OR (dim_date_won.date_actual IS NULL AND dim_date_lost.date_actual IS NULL))
              THEN 'Stage 6 - Awaiting Signature'
            WHEN dim_crm_touchpoint.bizible_touchpoint_date >= dim_date_won.date_actual 
              AND dim_crm_opportunity_flags.is_closed_won = TRUE
              THEN 'Stage 6 - Closed Won'
            WHEN dim_crm_touchpoint.bizible_touchpoint_date >= dim_date_lost.date_actual
    ...
    dim_crm_account.is_focus_partner,
      ...
      dim_date_sales_accepted.date_actual                                   AS sales_accepted_date,
      dim_date_close.date_actual                                            AS opportunity_close_date,
      dim_date_created.date_actual                                          AS opportunity_created_date,
      dim_date_arr_created.date_actual                                      AS pipeline_created_date,
      ...
      dim_crm_opportunity_source_and_path.sales_qualified_source            AS sales_qualified_source_name,
      dim_crm_opportunity_source_and_path.subscription_type,
      fct_crm_opportunity.closed_buckets,
      dim_crm_opportunity_source_and_path.source_buckets                    AS opportunity_source_buckets,
      ...
      dim_crm_opportunity.opportunity_owner_role                            AS opportunity_development_representative,
      ...
      -- bizible influenced
       CASE
        WHEN dim_campaign.budget_holder = 'fmm'
              OR campaign_owner.user_role_name = 'Field Marketing Manager'
    ...
    LEFT JOIN dim_date dim_date_stage0
      ON fct_crm_opportunity.stage_0_pending_acceptance_date_id = dim_date_stage0.date_id
    LEFT JOIN dim_date dim_date_stage1
      ON fct_crm_opportunity.stage_1_discovery_date_id = dim_date_stage1.date_id
    LEFT JOIN dim_date dim_date_stage2
      ON fct_crm_opportunity.stage_2_scoping_date_id = dim_date_stage2.date_id
    LEFT JOIN dim_date dim_date_stage3
      ON fct_crm_opportunity.stage_3_technical_evaluation_date_id = dim_date_stage3.date_id
    LEFT JOIN dim_date dim_date_stage4
      ON fct_crm_opportunity.stage_4_proposal_date_id = dim_date_stage4.date_id
    LEFT JOIN dim_date dim_date_stage5
      ON fct_crm_opportunity.stage_5_negotiating_date_id = dim_date_stage5.date_id
    LEFT JOIN dim_date dim_date_stage6
      ON fct_crm_opportunity.stage_6_awaiting_signature_date_id = dim_date_stage6.date_id
    LEFT JOIN dim_date dim_date_won
      ON fct_crm_opportunity.stage_6_closed_won_date_id = dim_date_won.date_id
    LEFT JOIN dim_date dim_date_lost
      ON fct_crm_opportunity.stage_6_closed_lost_date_id = dim_date_lost.date_id
    LEFT JOIN dim_date dim_date_sales_accepted
      ON fct_crm_opportunity.sales_accepted_date_id = dim_date_sales_accepted.date_id
    LEFT JOIN dim_date dim_date_close
      ON fct_crm_opportunity.close_date_id = dim_date_close.date_id
    LEFT JOIN dim_date dim_date_created
      ON fct_crm_opportunity.created_date_id = dim_date_created.date_id
    LEFT JOIN dim_date dim_date_arr_created
      ON fct_crm_opportunity.arr_created_date_id = dim_date_arr_created.date_id
