...
weekly_duo_seats AS (
    ...
    SELECT
        mart_arr_all_weekly.arr_week                                         AS reporting_week,
        --temporary field until we have the ability to calculate "billable installation" by week
        DATE_TRUNC('month', mart_arr_all_weekly.arr_week)                    AS reporting_month,
        mart_arr_all_weekly.subscription_name,
        mart_arr_all_weekly.dim_subscription_id,
        mart_arr_all_weekly.dim_subscription_id_original,
        mart_arr_all_weekly.crm_account_name,
        mart_arr_all_weekly.dim_crm_account_id,
        mart_arr_all_weekly.dim_parent_crm_account_id,
        mart_arr_all_weekly.product_deployment_type                          AS product_deployment,
        CASE
            WHEN LOWER(mart_arr_all_weekly.product_rate_plan_charge_name) LIKE '%duo%' 
                THEN SPLIT_PART(mart_arr_all_weekly.product_rate_plan_category, ' - ', 2)
        ...
        SUM(CASE WHEN add_on_name != 'GitLab Duo Core'
              THEN mart_arr_all_weekly.quantity
              ELSE 0 END)                                                    AS d_seats,   
        SUM(mart_arr_all_weekly.arr)                                         AS duo_arr,
        IFF((SUM(mart_arr_all_weekly.arr) > 0) 
            AND (add_on_name != 'GitLab Duo Core'), TRUE, FALSE)             AS is_duo_subscription_paid,
        mart_arr_all_weekly.turn_on_cloud_licensing,
        CASE 
            WHEN mart_arr_all_weekly.turn_on_cloud_licensing = 'Offline' THEN 'Offline Cloud License'
            WHEN mart_arr_all_weekly.turn_on_cloud_licensing = 'No' THEN 'Legacy License File'
            WHEN mart_arr_all_weekly.turn_on_cloud_licensing = 'Yes' THEN 'Standard Cloud License'
            WHEN mart_arr_all_weekly.turn_on_cloud_licensing = '' THEN 'Standard Cloud License'
            ELSE 'error'
        END                                                                  AS license_type,
    ...
    LEFT JOIN dim_crm_account
        ON mart_arr_all_weekly.dim_crm_account_id = dim_crm_account.dim_crm_account_id
    LEFT JOIN duo_core_enabled_subscriptions
        ON mart_arr_all_weekly.dim_subscription_id_original = duo_core_enabled_subscriptions.dim_subscription_id_original
        AND DATE_TRUNC('month', mart_arr_all_weekly.arr_week) = duo_core_enabled_subscriptions.snapshot_month
        AND mart_arr_all_weekly.product_delivery_type = duo_core_enabled_subscriptions.delivery_type
    WHERE mart_arr_all_weekly.arr_week BETWEEN '2024-02-18' AND CURRENT_DATE -- first duo pro arr
        AND (
          LOWER(mart_arr_all_weekly.product_rate_plan_charge_name) LIKE '%duo%'  -- Paid Duo add-ons
      OR (
          duo_core_enabled_subscriptions.is_duo_core_features_enabled = TRUE  -- Only include if Duo Core is enabled
          AND mart_arr_all_weekly.is_licensed_user_base_product = TRUE 
          AND (LOWER(mart_arr_all_weekly.product_tier_name) LIKE '%premium%' OR LOWER(mart_arr_all_weekly.product_tier_name) LIKE '%ultimate%')
      )
    )
    GROUP BY ALL
...
duo_tiers AS ( --tier occurring concurrently with paid duo pro subscription

    SELECT
        weekly_duo_seats.*,
        dim_product_detail.is_oss_or_edu_rate_plan,
        -- multiple product tiers can show up within the same ARR reporting week
        ARRAY_TO_STRING(ARRAY_AGG(DISTINCT SPLIT_PART(mart_arr_all_weekly.product_tier_name, ' - ', 2)), ', ')      AS paired_tier_name,
        -- not able to sort within group while using SPLIT_PART function - using this method for standard results
        IFF(paired_tier_name IN ('Premium, Ultimate', 'Ultimate, Premium'), 'Premium & Ultimate', paired_tier_name) AS clean_paired_tier
    FROM weekly_duo_seats
    LEFT JOIN mart_arr_all_weekly -- joining to get tier occuring within same week as add on
        ON weekly_duo_seats.reporting_week = mart_arr_all_weekly.arr_week
        AND weekly_duo_seats.dim_crm_account_id = mart_arr_all_weekly.dim_crm_account_id
        AND weekly_duo_seats.dim_subscription_id = mart_arr_all_weekly.dim_subscription_id -- add on will be on the same subscription as the tier
        AND mart_arr_all_weekly.is_licensed_user_base_product = TRUE
    LEFT JOIN dim_product_detail
        ON mart_arr_all_weekly.dim_product_detail_id = dim_product_detail.dim_product_detail_id
    GROUP BY ALL
...
sm_dedicated_duo_info AS ( --CTE purpose is to get product info about sm and dedicated instances even if they don't trigger AI Gateway events
    ...
    SELECT
        ...
        MAX(mart_ping_instance.major_minor_version_id)                                          AS major_minor_version_id,
        MAX(duo_tiers.d_seats)                                                                  AS duo_seats
    FROM duo_tiers
    LEFT JOIN map_installation_subscription_product
        ON duo_tiers.dim_subscription_id_original = map_installation_subscription_product.dim_subscription_id_original
        AND DATE_TRUNC(WEEK, map_installation_subscription_product.date_actual) = duo_tiers.reporting_week
    LEFT JOIN mart_ping_instance
        ...
        AND DATE_TRUNC(WEEK, map_installation_subscription_product.date_actual) = mart_ping_instance.ping_created_date_week
        AND mart_ping_instance.is_last_ping_of_week = TRUE
    WHERE duo_tiers.product_deployment IN ('Self-Managed', 'Dedicated')
    GROUP BY ALL
...
dotcom_duo_info AS ( --CTE purpose is to get product info about .com namespaces even if they don't trigger AI Gateway events
    ...
    LEFT JOIN dim_subscription
        ON dim_subscription.dim_subscription_id = duo_tiers.dim_subscription_id
    LEFT JOIN mart_ping_instance
        ON duo_tiers.reporting_week = mart_ping_instance.ping_created_date_week
        AND mart_ping_instance.dim_installation_id = '8b52effca410f0a380b0fcffaa1260e7' -- installation id for Gitlab.com
        AND mart_ping_instance.is_last_ping_of_week = TRUE
    WHERE duo_tiers.product_deployment = 'GitLab.com'
    GROUP BY ALL
...
duo_seat_assignments AS (
    /*
    When we roll up to the subscription level, we need to show the Duo billable users for the installation on the subscription 
    with the greatest number of _base tier_ billable users _in each week_. This mirrors our company licensing policy. 
    We could think of this installation as the "billable installation" on the subscription. Unfortunately, we don't currently 
    have a data model that makes it easy to find base tier billable users for anything but the last ping of the month. As such,
    we don't have a good way to select the "billable installation" on any given day. In order to get around this, 
    in this CTE, we assume that the "billable installation" at the end of the month was the "billable installation"
    for the entrie month.
    */
    SELECT
...
final AS (
    ...
    SELECT
        combined_duo_info.reporting_week,
    ...
    LEFT JOIN duo_seat_assignments
        ON TO_CHAR(duo_seat_assignments.product_entity_id) = TO_CHAR(combined_duo_info.product_entity_id)
    ...
    LEFT JOIN mart_behavior_structured_event_ai_gateway_flattened
        ...
        AND combined_duo_info.reporting_week = DATE_TRUNC(WEEK, mart_behavior_structured_event_ai_gateway_flattened.behavior_date)::DATE
    GROUP BY ALL
