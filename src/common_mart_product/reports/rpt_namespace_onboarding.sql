...
charges AS ( --First paid subscription for ultimate namespace
  ...
  SELECT DISTINCT
    ...
    charges.subscription_sales_type                                           AS first_paid_subscription_sales_type,
    IFF(
    ...
    IFF(charges.order_type = '1. First Order', '1. New - First Order', charges.order_type) AS first_paid_subscription_order_type
  FROM namespaces
  ...
  WHERE charges.is_licensed_user_base_product = TRUE --exclude add-ons, support, etc
    ...
    AND charges.mrr > 0
  QUALIFY ROW_NUMBER() OVER (PARTITION BY namespaces.ultimate_parent_namespace_id ORDER BY charges.subscription_start_date, charges.order_type) = 1
...
ai_gw AS (
  ...
  WHERE behavior_at::DATE >= '2024-08-03' --date of AI GW data becomes SSOT
...
feature_adoption AS (
  ...
  SELECT
    ...
    ai_gw.feature_category,
    MIN(behavior_at::DATE) AS first_feature_adoption_date
  FROM namespaces
  INNER JOIN ai_gw 
    ON namespaces.ultimate_parent_namespace_id = ai_gw.enabled_by_ultimate_parent_namespace_id
  GROUP BY ALL

),

feature_adoption_grouping AS (
  ...
  SELECT
    ...
    ARRAY_AGG(
      OBJECT_CONSTRUCT(
        'feature_category',
        feature_category,
        'first_feature_adoption_date',
        first_feature_adoption_date,
        'days_since_namespace_creation_at_feature_adoption_date',
        DATEDIFF('DAY', namespace_created_date, first_feature_adoption_date)
      )
    ) WITHIN GROUP (
      ORDER BY
        feature_category
    ) AS duo_features_adoption
...
base AS (
  ...
  SELECT DISTINCT
    ...
    charges.first_paid_subscription_sales_type,
    ...
    charges.first_paid_subscription_order_type,
    IFF(charges.first_paid_subscription_order_type = '1. New - First Order', TRUE, FALSE) AS first_paid_subscription_is_first_order,
    ...
    feature_adoption_grouping.duo_features_adoption,--array that includes all features' adoption per namespace
  ...
  LEFT JOIN feature_adoption_grouping
    ON namespaces.ultimate_parent_namespace_id = feature_adoption_grouping.ultimate_parent_namespace_id
