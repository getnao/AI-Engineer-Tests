...
subscription_level_calculations AS (
  ...
  SELECT DISTINCT
    ...
    production_usage_data.major_minor_version_num,
    ...
    (production_usage_data.ci_builds_per_billable_user) * (monthly_subscription_base_products_arr.total_subscription_base_products_arr)																							AS subscription_ci_builds_per_billable_user_dollars,
...
production_usage_data.duo_amazon_q_license_user_count,
production_usage_data.duo_amazon_q_billable_user_count,
    production_usage_data.duo_amazon_q_license_utilization,
...
account_rollup_calculations AS (
  ...
  SELECT
    ...
    CASE WHEN account_weighted_scm_utilization > 0.33 THEN 'Green'
      WHEN account_weighted_scm_utilization > 0.10 AND account_weighted_scm_utilization <= 0.33 THEN 'Yellow'
      WHEN account_weighted_scm_utilization <= 0.10 THEN 'Red'
      ELSE 'No Data'
    ...
    CASE WHEN account_weighted_ci_utilization > 0.333 THEN 'Green'
      ...
      ELSE 'No Data'
    END                                                                                                                                                              AS old_account_level_ci_color,
    CASE WHEN account_weighted_ci_builds_per_billable_user > 40 THEN 'Green'
      WHEN account_weighted_ci_builds_per_billable_user > 2 AND account_weighted_ci_builds_per_billable_user <= 40 THEN 'Yellow'
      WHEN account_weighted_ci_builds_per_billable_user <= 2 THEN 'Red'
      ELSE 'No Data'
...
final AS (
  ...
  SELECT
    rpt_product_usage_health_score.* EXCLUDE (primary_key),
