...
{{ simple_cte([
    ...
    ('gitlab_dotcom_namespace_settings_snapshots_base', 'gitlab_dotcom_namespace_settings_snapshots_base'),
...
-- Only licence users are avaiable from zuora
  ...
  SELECT
    ...
    SUM(IFF(SPLIT_PART(prep_product_detail.product_rate_plan_category, ' - ', 2) = 'GitLab Duo with Amazon Q', prep_charge_mrr_daily.quantity, NULL))  AS duo_amazon_q_license_users_zuora
  ...
  WHERE prep_product_detail.is_licensed_user = TRUE
    ...
    AND SPLIT_PART(prep_product_detail.product_rate_plan_category, ' - ', 2) IN ('GitLab Duo Pro', 'GitLab Duo Enterprise', 'Enterprise Agile Planning', 'GitLab Duo with Amazon Q')
...
seat_link_base AS (
  ...
  SELECT
    report_date,
    dim_installation_id,
    MAX(duo_pro_license_users)          AS duo_pro_license_users_seat_link,
    MAX(duo_pro_billable_users)         AS duo_pro_billable_users_seat_link,
    MAX(duo_enterprise_license_users)   AS duo_enterprise_license_users_seat_link,
    MAX(duo_enterprise_billable_users)  AS duo_enterprise_billable_users_seat_link,
    MAX(duo_amazon_q_license_users)     AS duo_amazon_q_license_users_seat_link,
    MAX(duo_amazon_q_billable_users)    AS duo_amazon_q_billable_users_seat_link
...
service_ping_base AS (
  ...
  SELECT
    ...
    duo_amazon_q_purchased_seats             AS duo_amazon_q_license_users_service_ping,
    duo_amazon_q_assigned_seats              AS duo_amazon_q_billable_users_service_ping
  ...
  WHERE dim_installation_id IS NOT NULL
    AND COALESCE(
      ...
      duo_amazon_q_purchased_seats,
      duo_amazon_q_assigned_seats,
      is_duo_core_features_enabled
    ) IS NOT NULL
...
dotcom_add_on_purchases AS (
  ...
  SELECT 
    ...
    purchases_snapshot.corrected_valid_from_date,
    purchases_snapshot.corrected_valid_to_date
...
dotcom_add_on_daily_purchases_assignments AS ( -- CTE to calculate daily assigned seats per namespace ID
  ...
  CROSS JOIN dim_date
  WHERE 
    -- Only consider rows within the snapshot's effective time frame to avoid double-counting assignments across version
    dim_date.date_actual BETWEEN DATE(purchases_snapshot.corrected_valid_from_date)
                              AND COALESCE(purchases_snapshot.corrected_valid_to_date, CURRENT_DATE())
  GROUP BY ALL
...
namespaces_with_duo_core_enabled AS (
  SELECT DISTINCT
    dim_date.date_day,
    dim_namespace_id,
    is_duo_core_features_enabled
  FROM gitlab_dotcom_namespace_settings_snapshots_base
  INNER JOIN dim_date
    ON dim_date.date_day >= valid_from
    AND (dim_date.date_day < valid_to OR valid_to IS NULL)
    AND dim_date.date_day <= CURRENT_DATE  -- Limit to current date
  WHERE is_duo_core_features_enabled = TRUE  -- Only get records where the flag is TRUE
),
...
joined AS (
  ...
  SELECT
    COALESCE(
      ...
      namespaces_with_duo_core_enabled.date_day
    ...
    COALESCE(
      ...
      namespaces_with_duo_core_enabled.dim_namespace_id
    )::INT                                                                     AS dim_namespace_id,

    IFF(prep_namespace.namespace_is_internal = TRUE, TRUE, FALSE)::BOOLEAN     AS namespace_is_internal,

    COALESCE(
      service_ping_base.is_duo_core_features_enabled,
      namespaces_with_duo_core_enabled.is_duo_core_features_enabled,
      FALSE  -- Default to FALSE when not found in either source
    )::BOOLEAN                                                                 AS is_duo_core_features_enabled,
    ...
    -- Duo with Amazon Q License Users
    COALESCE(
      zuora_base.duo_amazon_q_license_users_zuora,
      service_ping_base.duo_amazon_q_license_users_service_ping,
      seat_link_base.duo_amazon_q_license_users_seat_link
    )::INT                                                                     AS duo_amazon_q_license_users_coalesced,

    zuora_base.duo_amazon_q_license_users_zuora::INT                           AS duo_amazon_q_license_users_zuora,
    seat_link_base.duo_amazon_q_license_users_seat_link::INT                   AS duo_amazon_q_license_users_seat_link,
    service_ping_base.duo_amazon_q_license_users_service_ping::INT             AS duo_amazon_q_license_users_service_ping,
    ...
    -- Duo with Amazon Q Billable Users

    COALESCE(
      service_ping_base.duo_amazon_q_billable_users_service_ping,
      seat_link_base.duo_amazon_q_billable_users_seat_link
    )::INT                                                                     AS duo_amazon_q_billable_users_coalesced,

    seat_link_base.duo_amazon_q_billable_users_seat_link::INT                  AS duo_amazon_q_billable_users_seat_link,
    service_ping_base.duo_amazon_q_billable_users_service_ping::INT            AS duo_amazon_q_billable_users_service_ping
  ...
  FULL OUTER JOIN namespaces_with_duo_core_enabled
    ON COALESCE(zuora_base.reporting_date, dotcom_assignment_base.reporting_date) = namespaces_with_duo_core_enabled.date_day 
      AND COALESCE(zuora_base.dim_namespace_id, dotcom_assignment_base.dim_namespace_id) = namespaces_with_duo_core_enabled.dim_namespace_id
...
SELECT 
  ...
  joined.is_duo_core_features_enabled,
  ...
  joined.duo_amazon_q_license_users_coalesced,
  joined.duo_amazon_q_license_users_zuora,
  joined.duo_amazon_q_license_users_seat_link,
  joined.duo_amazon_q_license_users_service_ping,
  ...
  joined.duo_amazon_q_billable_users_coalesced,
  joined.duo_amazon_q_billable_users_seat_link,
  joined.duo_amazon_q_billable_users_service_ping
