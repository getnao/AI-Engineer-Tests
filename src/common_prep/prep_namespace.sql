...
joined AS (
  ...
  SELECT
    ...
    COALESCE(namespace_details.creator_id, creators.creator_id, namespaces.owner_id)                     AS creator_id,
    ...
    -- Attributes
    IFF(namespaces.namespace_id = COALESCE(
      namespace_lineage.ultimate_parent_id,
      namespaces.parent_id,
      namespaces.namespace_id
    ),
    ...
    namespace_settings.is_duo_features_enabled,
    namespace_settings.is_duo_core_features_enabled,
    namespace_settings.is_lock_duo_features_enabled,
    namespace_settings.experiment_features_enabled,
    CASE
      WHEN NOT is_duo_features_enabled AND is_lock_duo_features_enabled AND NOT experiment_features_enabled THEN 'always_off'
      WHEN is_duo_features_enabled AND NOT is_lock_duo_features_enabled THEN 'default_on'
      WHEN NOT is_duo_features_enabled AND NOT is_lock_duo_features_enabled THEN 'default_off'
    END                                                                                                  AS duo_availability,
  ...
  LEFT JOIN namespaces AS internal_namespaces
    ON namespace_lineage.ultimate_parent_id = internal_namespaces.namespace_id
  LEFT JOIN namespace_details
    ON namespaces.namespace_id = namespace_details.namespace_id
  LEFT JOIN product_tiers AS saas_product_tiers
    ON saas_product_tiers.product_deployment_type = 'GitLab.com'
      AND namespace_lineage.ultimate_parent_plan_name = LOWER(IFF(
