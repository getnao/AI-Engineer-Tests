...
WITH dim_billing_account AS (
  ...
  {{ dbt_utils.group_by(n=6) }}
    ...
    SELECT
      ...
      dim_subscription.dim_crm_opportunity_id                                         AS dim_crm_opportunity_id,
      dim_subscription.dim_crm_opportunity_id_original                                AS dim_crm_opportunity_id_original,
    ...
    LEFT JOIN dim_crm_opportunity
      ON dim_crm_opportunity.dim_crm_opportunity_id = dim_subscription.dim_crm_opportunity_id
  ...
  FROM joined
    ...
    SELECT
      cohort_diffs.*,
      CASE
        WHEN cohort_diffs.months_since_parent_account_cohort_start = 0 AND cohort_diffs.is_oss_or_edu_rate_plan = FALSE 
          THEN TRUE
        ELSE FALSE
      END                                        AS is_landed_arr_first_order, 
      arr_band_calc
    FROM cohort_diffs
