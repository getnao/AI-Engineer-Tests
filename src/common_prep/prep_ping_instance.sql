...
, source AS (
...
), combined_ping_with_subscription_id AS (

  SELECT
    combined_ping.*,
    COALESCE(sha256.dim_subscription_id, md5.dim_subscription_id)                    AS dim_subscription_id_license,
    COALESCE(sha256.dim_license_id, md5.dim_license_id)                              AS dim_license_id
  FROM combined_ping
  LEFT JOIN prep_license AS md5
    ON combined_ping.license_md5 = md5.license_md5
  LEFT JOIN prep_license AS sha256
    ON combined_ping.license_sha256 = sha256.license_sha256

), self_managed_product_tier AS (
  ...
  WHERE product_delivery_type = 'Self-Managed'
    ...
    SELECT
      combined_ping_with_subscription_id.*,
      ...
      raw_usage_data_payload:duo_seats.amazon_q_purchased::NUMBER                                                                                     AS duo_amazon_q_purchased_seats,
      /*
      Since there is no seat assignment for Gitlab Duo with Amazon Q at launch and the product is sold 1:1 with base tier seats,
      we assume the Duo with Amazon Q billable seats are equal to the base tier billable seats.
      */
      IFF(duo_amazon_q_purchased_seats IS NOT NULL, license_billable_users, NULL)                                                                     AS duo_amazon_q_assigned_seats,
      IFF(raw_usage_data_payload:settings.duo_core_features_enabled = '-1', NULL,raw_usage_data_payload:settings.duo_core_features_enabled)::BOOLEAN  AS is_duo_core_features_enabled,
      CASE
        WHEN internal_installations.dim_installation_id IS NOT NULL THEN TRUE
        WHEN combined_ping_with_subscription_id.installation_type = 'gitlab-development-kit' THEN TRUE
        ELSE FALSE
      END                                                                                                                                             AS is_internal,
      CASE
        WHEN combined_ping_with_subscription_id.hostname ILIKE 'staging.%'
          OR combined_ping_with_subscription_id.hostname = 'dr.gitlab.com' THEN TRUE
        ELSE FALSE 
      END                                                                                                                                             AS is_staging,
      COALESCE(license_subscription_id, combined_ping_with_subscription_id.dim_subscription_id_license)                                               AS dim_subscription_id,
      self_managed_product_tier.dim_product_tier_id

    FROM combined_ping_with_subscription_id
    LEFT JOIN internal_installations
      ON combined_ping_with_subscription_id.dim_installation_id = internal_installations.dim_installation_id
    LEFT JOIN self_managed_product_tier
      ON combined_ping_with_subscription_id.product_tier = self_managed_product_tier.product_tier_name_short
