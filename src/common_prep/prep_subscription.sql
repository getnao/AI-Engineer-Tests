...
zuora_subscription_original AS (

  SELECT *
  FROM {{ ref('zuora_subscription_source') }}
  WHERE is_deleted = FALSE
    AND exclude_from_analysis IN ('False', '')
    AND zuora_subscription_source.version = 1
...
joined AS (
  ...
  SELECT
    ...
    zuora_subscription_original.sfdc_opportunity_id                                                                                                                                               AS dim_crm_opportunity_id_original,
    ...
    CASE
      WHEN prep_billing_account_user.is_self_service_integration_user = 1
        THEN 'Self-Service'
      ELSE 'Sales-Assisted'
    ...
    COALESCE (zuora_subscription.ramp_id != '' OR (zuora_subscription.multi_year_deal_subscription_linkage != '' AND zuora_subscription.multi_year_deal_subscription_linkage IS NOT NULL AND zuora_subscription.multi_year_deal_subscription_linkage != 'Not a ramp'), FALSE) AS is_ramp,
    ...
    CASE
      WHEN LOWER(zuora_subscription.subscription_status) = 'active' AND zuora_subscription.subscription_end_date > CURRENT_DATE
    ...
    zuora_subscription.is_trial_subscription
  ...
  LEFT JOIN zuora_subscription_original
    ON zuora_subscription_original.original_id = zuora_subscription.original_id
