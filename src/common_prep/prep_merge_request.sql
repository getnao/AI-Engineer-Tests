...
agg_labels AS (
  ...
  SELECT
    gitlab_dotcom_merge_requests_source.merge_request_id,
...
renamed AS (
  ...
  SELECT
    ...
    COALESCE(prep_namespace_plan_hist.dim_plan_id, 34)                                                                                AS dim_plan_id_at_creation,
    ...
    IFF(
      target_project.visibility_level != 'public' AND target_project.namespace_is_internal = FALSE,
      'content masked', gitlab_dotcom_merge_requests_source.merge_request_title
    )                                                                                                                                 AS merge_request_title,
    IFF(
      target_project.visibility_level != 'public' AND target_project.namespace_is_internal = FALSE,
      'content masked', gitlab_dotcom_merge_requests_source.merge_request_description
    )                                                                                                                                 AS merge_request_description,
    ...
    gitlab_dotcom_merge_requests_source.merge_commit_sha,
    gitlab_dotcom_merge_requests_source.rebase_commit_sha,
    gitlab_dotcom_merge_requests_source.sprint_id,
    gitlab_dotcom_merge_requests_source.draft                                                                                         AS is_draft,
    gitlab_dotcom_merge_requests_source.prepared_at,
    gitlab_dotcom_merge_requests_source.imported_from,
    gitlab_dotcom_merge_requests_source.retargeted                                                                                    AS is_retargeted,
    gitlab_dotcom_merge_requests_source.override_requested_changes                                                                    AS is_override_requested_changes,
    ...
    IFF(
      gitlab_dotcom_merge_requests_source.target_project_id IN ({{ is_project_included_in_engineering_metrics() }}),
      TRUE, FALSE
    )                                                                                                                                 AS is_included_in_engineering_metrics,
    IFF(
      gitlab_dotcom_merge_requests_source.target_project_id IN ({{ is_project_part_of_product() }}),
      TRUE, FALSE
    )                                                                                                                                 AS is_part_of_product,
    IFF(
      target_project.namespace_is_internal IS NOT NULL
      AND ARRAY_CONTAINS(CAST('community contribution' AS VARIANT), agg_labels.labels),
      TRUE, FALSE
    )                                                                                                                                 AS is_community_contributor_related,
    target_project.namespace_is_internal,
    gitlab_dotcom_merge_requests_source.target_branch,
    ...
    TIMESTAMPDIFF(
      HOURS, gitlab_dotcom_merge_request_metrics_source.created_at,
      gitlab_dotcom_merge_request_metrics_source.merged_at
  ...
  LEFT JOIN prep_gitlab_dotcom_plan
    ON COALESCE(prep_namespace_plan_hist.dim_plan_id, 34) = prep_gitlab_dotcom_plan.dim_plan_id
  ...
  LEFT JOIN prep_user AS merge_user
