...
cortex_functions AS (
  SELECT 
    query_id,
    SUM(cortex_function_tokens) AS query_cortex_function_tokens,
    SUM(cortex_function_token_credits) AS query_cortex_function_token_credits,
    ARRAY_AGG(OBJECT_CONSTRUCT(
      'cortex_model_name', NULLIF(cortex_model_name,''),
      'cortex_function_name', cortex_function_name, 
      'cortex_function_tokens', cortex_function_tokens, 
      'cortex_function_token_credits', cortex_function_token_credits)
    ) AS query_cortex_functions
  FROM {{ ref('snowflake_cortex_functions_query_usage_history_source') }}
  GROUP BY 1
),
...
expanded AS (
  ...
  SELECT
    ...
    (credit_rates.contract_rate * 
        cortex_functions.query_cortex_function_token_credits)::NUMBER(38,2)               AS cortex_function_dollars_spent,
    cortex_functions.query_cortex_function_tokens,
    cortex_functions.query_cortex_function_token_credits,
    cortex_functions.query_cortex_functions,
  ...
  LEFT JOIN cortex_functions
    ON source.query_id = cortex_functions.query_id
