...
code_suggestion_context AS (
  ...
  SELECT
    ...
    fct_behavior_structured_event.unique_instance_id,
    ...
    fct_behavior_structured_event.resolution_strategies,
    ...
    fct_behavior_structured_event.deployment_type,
    fct_behavior_structured_event.feature_enablement_type
...
flattened_namespaces AS (
  ...
  SELECT
    ...
    code_suggestion_context.deployment_type,
...
code_suggestions_with_ultimate_parent_namespaces_and_crm_accounts AS (
  ...
  LEFT JOIN prep_namespace
    ON flattened_namespaces.namespace_id = prep_namespace.namespace_id 
      AND flattened_namespaces.deployment_type = 'GitLab.com'
  LEFT JOIN prep_namespace_w_bdg
    ON prep_namespace.namespace_id = prep_namespace_w_bdg.dim_namespace_id
...
code_suggestions_with_multiple_ultimate_parent_crm_accounts_saas AS (
  ...
  SELECT
    ...
    ARRAY_UNIQUE_AGG(ultimate_parent_namespace_id) AS ultimate_parent_namespace_ids,
    ARRAY_UNIQUE_AGG(subscription_name)            AS subscription_names,
    ARRAY_UNIQUE_AGG(dim_crm_account_id)           AS dim_crm_account_ids,
    ARRAY_UNIQUE_AGG(dim_parent_crm_account_id)    AS dim_parent_crm_account_ids,
...
code_suggestions_with_multiple_ultimate_parent_crm_accounts_sm AS (
  ...
  SELECT
    ...
    ARRAY_UNIQUE_AGG(context_with_installation_id.dim_installation_id)           AS dim_installation_ids,
    ARRAY_UNIQUE_AGG(context_with_installation_id.host_name)                     AS host_names,
    ARRAY_UNIQUE_AGG(context_with_installation_id.product_delivery_type)         AS product_delivery_types,
    ARRAY_UNIQUE_AGG(context_with_installation_id.product_deployment_type)       AS product_deployment_types,
    ARRAY_UNIQUE_AGG(bdg_latest_instance_subscription.subscription_name)         AS subscription_names,
    ARRAY_UNIQUE_AGG(bdg_latest_instance_subscription.dim_crm_account_id)        AS dim_crm_account_ids,
    ARRAY_UNIQUE_AGG(bdg_latest_instance_subscription.dim_parent_crm_account_id) AS dim_parent_crm_account_ids,
