...
order_type_of_decommissioned_opportunity AS (

  SELECT *
  FROM {{ ref('prep_crm_opportunity_order_type_decommissioned_opportunity_ids') }}
...
calculations AS (
  ...
  SELECT
    ...
    {{ dbt_utils.star(
        from=ref('sfdc_opportunity_snapshots_source'), 
        except=['created_date', 'sales_accepted_date', 'close_date', 'deal_path', 'opportunity_id', 'owner_id', 'account_id', 'parent_opportunity_id', 'net_arr', 'opportunity_term', 'is_won', 'sdc_batched_at', 'sdc_received_at', 'sdc_sequence', 'sdc_extracted_at', 'ultimate_parent_sales_segment_emp', 'sales_segmentation_employees', 'new_logo_count', 'dbt_valid_from', 'dbt_valid_to'],
      ) 
    }},
    CASE
    ...
    {{ sales_qualified_source_cleaning('generated_source', 'opportunity_category') }}                               AS sales_qualified_source,
    ...
    -- replicate the updated new_logo_count logic for historical data https://gitlab.com/gitlab-com/sales-team/field-operations/systems/-/issues/7757
    CASE
      WHEN
        order_type_stamped  = '1. New - First Order'
        AND is_edu_oss != 1
        AND (opportunity_category != 'Decommission' OR opportunity_category IS NULL)
      THEN 1
      WHEN
        decommissioned_opps_order_type.order_type_of_decommissioned_opportunity = '1. New - First Order'
      THEN -1
      ELSE 0
    END                                                                                                             AS calculated_new_logo_count,

    -- to calculate the new_logo_count for historic records, use calculated_new_logo_count https://gitlab.com/gitlab-data/analytics/-/issues/23173
    CASE
      WHEN
        -- adding some buffer to the definition implementation date due to the need to reset Stitch afterwards 
        updated_source_with_net_arr_logic.dbt_valid_from <= '2025-03-22' 
      THEN calculated_new_logo_count
      ELSE new_logo_count
    END                                                                                                             AS new_logo_count,

    CASE
      WHEN 
        opportunity_category IN ('Standard', 'Ramp Deal', 'Internal Correction', 'Contract Reset/Ramp Deal')
        AND stage_name NOT IN ('0-Pending Acceptance', '9-Unqualified', '10-Duplicate')
        AND stage_1_discovery_date IS NOT NULL
        AND (crm_business_dev_rep_id IS NOT NULL OR crm_sales_dev_rep_id IS NOT NULL)
      THEN TRUE
      ELSE FALSE
    END                                                                                                             AS is_sales_dev_qualified_opportunity,

    --metadata 
    updated_source_with_net_arr_logic.dbt_valid_from,
    updated_source_with_net_arr_logic.dbt_valid_to,
  ...
  LEFT JOIN order_type_of_decommissioned_opportunity AS decommissioned_opps_order_type
    ON updated_source_with_net_arr_logic.opportunity_id = decommissioned_opps_order_type.opportunity_id
    AND updated_source_with_net_arr_logic.dbt_valid_from = decommissioned_opps_order_type.dbt_valid_from
    AND {{ coalesce_to_infinity('updated_source_with_net_arr_logic.dbt_valid_to') }} 
        = {{ coalesce_to_infinity('decommissioned_opps_order_type.dbt_valid_to') }}

),

calculated_order_type AS (
  ...
  SELECT
    *,
    ...
    CASE 
      ...
      ELSE order_type_stamped
    END                                                                                                             AS order_type, 

    CASE
      WHEN order_type = '1. New - First Order'
        THEN '1) New - First Order'
      WHEN order_type IN ('2. New - Connected', '3. Growth', '4. Contraction', '5. Churn - Partial', '6. Churn - Final')
        THEN '2) Growth (Growth / New - Connected / Churn / Contraction)'
      WHEN order_type IN ('7. PS / Other')
