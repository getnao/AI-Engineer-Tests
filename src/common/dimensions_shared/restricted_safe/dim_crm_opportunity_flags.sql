{{ config({
    "tags": ["six_hourly"],
    "post-hook": "{{ missing_member_column(primary_key = 'dim_crm_opportunity_flags_sk', not_null_test_cols = []) }}"
    })
}}
...
WITH prep_crm_opportunity AS (
    ...
    SELECT DISTINCT -- note that we get the distinct combinations of flags to prevent fanning when this table is joined upon
      ...
      is_booked_net_arr,
      is_win_rate_calc,
      is_sales_dev_qualified_opportunity,
      is_sales_dev_pipeline_created,
      ...
      -- Deal related
      is_sdr_sao,
      is_net_arr_pipeline_created_snapshot,
      is_swing_deal,
      is_net_most_likely_opportunity,
