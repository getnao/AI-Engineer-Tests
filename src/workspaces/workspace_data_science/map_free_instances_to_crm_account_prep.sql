...
{%- if is_incremental() %}
    {%- set day_check_query %}
        SELECT 
            CASE 
                WHEN DAYNAME(CURRENT_DATE) = '{{ var("MODEL_RUN_DAY", "Tue") }}' THEN true  -- run on Tuesday. Or the day provided to the variable MODEL_RUN_DAY
                WHEN DAYNAME(CURRENT_DATE) IN ('Wed', 'Thu', 'Fri', 'Sat', 'Sun', 'Mon')  -- If it didn't run on Tuesday, run the next time the DAG runs (later in the week)
                    AND DATE_TRUNC('week', CURRENT_DATE) <> (
                        SELECT DATE_TRUNC('week', MAX(processed_date)) 
                        FROM {{ this }}
                    ) THEN true
                ELSE false
            END AS should_run
