...
{% docs fct_targets_actuals_daily_snapshot %}

Derived fact table from `fct_crm_opportunity_daily_snapshot` and `fct_sales_funnel_target_pivoted`. This fct table simplifies tracking sales performance by showing targets and actuals combined.
...
{% docs fct_team_member_org_history %}

Contains the team member's history for department, division, cost center, entity, and employee type.
...
{% docs dim_channel_type %}

Dimension table representing each Channel Type. Channel Type is derived from `sqs_bucket_engagement` and `order_type` of each Opportunity.

{% enddocs %}

{% docs dim_partner_category %}

Dimension table representing each Partner Category. Partner Category is derived from `sales_qualified_source` and `fulfillment_partner_name`.

{% enddocs %}

{% docs fct_gcp_billing_line_item %}

GCP billing line items with credits

{% enddocs %}

{% docs fct_license_utilization %}

This fact model consolidates license and billable user data from multiple sources including Zuora, 
Seat Link, Service Ping, GitLab.com add-on assignments, and GitLab.com subscriptions. It provides 
a unified view of license utilization across all GitLab deployment types and product features.
