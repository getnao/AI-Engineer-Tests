...
SELECT 
    ...
    CASE
        ...
        WHEN LOWER(medium) = 'referral'
          AND (CONTAINS(LOWER(source), 'chatgpt.com')
            OR CONTAINS(LOWER(source), 'perplexity')
            OR CONTAINS(LOWER(source), 'edgepilot')
            OR CONTAINS(LOWER(source), 'edgeservices')
            OR CONTAINS(LOWER(source), 'copilot.microsoft.com')
            OR CONTAINS(LOWER(source), 'openai.com')
            OR CONTAINS(LOWER(source), 'gemini.google.com')
            OR CONTAINS(LOWER(source), 'nimble.ai')
            OR CONTAINS(LOWER(source), 'iask.ai')
            OR CONTAINS(LOWER(source), 'claude.ai')
            OR CONTAINS(LOWER(source), 'aitastic.app')
            OR CONTAINS(LOWER(source), 'bnngpt.com')
            OR CONTAINS(LOWER(source), 'writesonic.com')
            OR CONTAINS(LOWER(source), 'copy.ai')
            OR CONTAINS(LOWER(source), 'chat-gpt.org')
            OR REGEXP_LIKE(LOWER(source), 'chat\\.[^.]+\\.ai')
            OR REGEXP_LIKE(LOWER(source), '\\.ai\\/chat')
            OR CONTAINS(LOWER(source), 'gpt')
            )
        THEN 'ai referral'
...
WHERE session_engaged IN ('0','1')
  AND (ga_session_id REGEXP '^[0-9]+$' OR ga_session_id IS NULL)  
  AND (ga_session_number REGEXP '^[0-9]+$' OR ga_session_number IS NULL)
  AND (user_pseudo_id != '40281114.1738746614' OR user_pseudo_id IS NULL)