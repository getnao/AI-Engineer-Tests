...
final as (
  ...
  SELECT
    ...
    CASE
    -- only calculate Base segments for FY26+ opps. 
    -- Base prospect accounts may have old closed lost opps, or old churned customers with associated opps. Those are not base.
      WHEN close_fiscal_year < 2026
        OR close_fiscal_year IS NULL
        THEN dim_crm_user_hierarchy.crm_user_sales_segment
    -- capturing all opps which are being worked on by the base team in F26
      WHEN dim_crm_user_hierarchy.crm_user_region = 'BASE' 
        THEN 'BASE - MID-MARKET'
    -- base segments are only built on smb or null report segments, and in FY26
      WHEN dim_crm_user_hierarchy.crm_user_sales_segment != 'SMB' 
        AND dim_crm_user_hierarchy.crm_user_sales_segment IS NOT NULL
        THEN dim_crm_user_hierarchy.crm_user_sales_segment
      WHEN (dim_crm_user_hierarchy.crm_user_sales_segment = 'SMB'
        OR dim_crm_user_hierarchy.crm_user_sales_segment IS NULL)
        AND dim_crm_account.is_base_prospect_account = TRUE
        AND dim_crm_account.parent_crm_account_max_family_employee > 4000 
        AND (dim_crm_account.it_spend <= 250000 OR dim_crm_account.it_spend IS NULL)
        AND close_fiscal_year >= 2026
        THEN 'BASE - LARGE'
      WHEN (dim_crm_user_hierarchy.crm_user_sales_segment = 'SMB'
        OR dim_crm_user_hierarchy.crm_user_sales_segment IS NULL)
        AND dim_crm_account.is_base_prospect_account = TRUE
        AND (dim_crm_account.parent_crm_account_max_family_employee <= 4000 
          OR dim_crm_account.parent_crm_account_max_family_employee IS NULL)
        AND close_fiscal_year >= 2026
        THEN 'BASE - MID-MARKET'
         -- if the base prospect box is not true (F/Null), then FY26 opps need to go into SMB - minus base
      WHEN dim_crm_user_hierarchy.crm_user_sales_segment = 'SMB' 
        AND (dim_crm_account.is_base_prospect_account = FALSE
          OR dim_crm_account.is_base_prospect_account IS NULL)
        AND close_fiscal_year >= 2026
    ...
    fct_crm_opportunity.potential_seat_count,
