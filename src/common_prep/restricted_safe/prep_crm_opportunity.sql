...
{{ simple_cte([
    ...
    ('base_opportunity_hierarchy_sk_override_fy26', 'base_opportunity_hierarchy_sk_override_fy26')
]) }},
...
sao_base AS (
  ...
  SELECT
    ...
    IFF(base.is_edu_oss = FALSE AND base.stage_name != '10-Duplicate' AND sales_accepted_date BETWEEN valid_from AND valid_to, TRUE, FALSE) AS is_abm_tier_sao
  FROM base
...
final AS (
  ...
  SELECT
    ...
    -- net arr
    CASE
      WHEN sfdc_opportunity_stage.is_won = TRUE -- only consider won deals
    ...
    IFF(base.opportunity_category_live = 'Decommission', TRUE, FALSE)                                 AS is_decommissioned,
    COALESCE(
      base.sales_accepted_date IS NOT NULL
      AND base.is_edu_oss_live = FALSE
    ...
    /*
      ...
      We are updating is_net_arr_pipeline_created so that only snapshot fields are used for the calculation, instead of mixing snapshot and live fields.
      https://gitlab.com/gitlab-data/analytics/-/issues/24022
    */
    CASE
      WHEN base.order_type IN ('1. New - First Order', '2. New - Connected', '3. Growth')
        AND base.is_edu_oss = FALSE
        AND arr_created_date.first_day_of_fiscal_quarter IS NOT NULL
        AND base.opportunity_category IN ('Standard', 'Internal Correction', 'Ramp Deal', 'Credit', 'Contract Reset', 'Contract Reset/Ramp Deal')
        AND base.stage_name NOT IN ('00-Pre Opportunity', '10-Duplicate', '9-Unqualified', '0-Pending Acceptance')
        AND (
          net_arr > 0
          OR base.opportunity_category = 'Credit'
        )
        AND base.sales_qualified_source != 'Web Direct Generated'
        AND (base.is_jihu_account != TRUE OR base.is_jihu_account IS NULL)
        AND base.stage_1_discovery_date IS NOT NULL
        THEN TRUE
      ELSE FALSE
    END                                                                                               AS is_net_arr_pipeline_created,
    /*
    A snapshot version of is_net_arr_pipeline_created to support XDR Comp reporting
    https://gitlab.com/gitlab-data/analytics/-/issues/23163

    -- to be deprecated with updating is_net_arr_pipeline_created with snapshot fields instead of using live fields
    */
    CASE
      WHEN base.order_type IN ('1. New - First Order', '2. New - Connected', '3. Growth')
        AND base.is_edu_oss = FALSE
        ...
        THEN TRUE
      ELSE FALSE
    END                                                                                               AS is_net_arr_pipeline_created_snapshot,
    COALESCE(
    ...
    CASE
      WHEN base.is_edu_oss_live = FALSE
        AND base.is_deleted = FALSE
    ...
    CASE
      WHEN base.is_edu_oss_live = FALSE
        ...
        AND (
          sfdc_opportunity_stage.is_won = TRUE
          OR (base.is_renewal = TRUE AND is_lost = TRUE)
        )
        AND base.order_type_live IN ('1. New - First Order', '2. New - Connected', '3. Growth', '4. Contraction', '6. Churn - Final', '5. Churn - Partial')
        THEN TRUE
      ELSE FALSE
    ...
    IFF(base.opportunity_category_live IN ('Credit'), TRUE, FALSE)                                    AS is_credit,
    IFF(base.opportunity_category_live IN ('Contract Reset'), TRUE, FALSE)                            AS is_contract_reset,
    ...
    -- To be deprecated: deal_size, calculated_deal_size (deal_size_bucket is now the SSOT field)
    CASE
    ...
    -- new SSOT deal size bucketing aligned with Sales Systems https://gitlab.com/gitlab-com/sales-team/field-operations/systems/-/issues/5720  
    CASE
      WHEN net_arr > 0 AND net_arr < 5000
        THEN '1. <$5K'
      WHEN net_arr >= 5000 AND net_arr < 30000
        THEN '2. $5K-$30K'
      WHEN net_arr >= 30000 AND net_arr < 100000
        THEN '3. $30K-$100K'
      WHEN net_arr >= 100000 AND net_arr < 300000
        THEN '4. $100K-$300K'
      WHEN net_arr >= 300000 
        THEN '5. $300K+'
      ELSE 'Other'
    ...
    CASE
      WHEN net_arr > -5000
        AND is_eligible_churn_contraction = TRUE
        THEN '1. < 5k'
      WHEN net_arr > -20000
        AND net_arr <= -5000
        AND is_eligible_churn_contraction = TRUE
    ...
    CASE
      WHEN (
        ...
        OR sfdc_opportunity_stage.is_won = TRUE
      )
    ...
    CASE -- Only two SQS target values from FY26 https://gitlab.com/gitlab-data/analytics/-/issues/22166
      WHEN close_fiscal_year >= 2026 
        AND base.sales_qualified_source != 'SDR Generated'
          THEN 'Non-SDR Generated'
      WHEN is_sao = TRUE -- account for 'Stage 1 Opportunities' in actuals
        AND sales_accepted_date >= '2025-02-01' -- FY26
        AND base.sales_qualified_source != 'SDR Generated'
          THEN 'Non-SDR Generated'
      WHEN is_net_arr_pipeline_created = TRUE -- account for 'Net ARR Pipeline Created' in actuals
        AND arr_created_date >= '2025-02-01' -- FY26
        AND base.sales_qualified_source != 'SDR Generated'
          THEN 'Non-SDR Generated'
      ELSE base.sales_qualified_source 
    END                                                                                               AS sales_qualified_source_name_simplified,
    {{ dbt_utils.generate_surrogate_key(['sales_qualified_source_name_simplified']) }}                AS dim_sales_qualified_source_id_simplified,
    ...
    IFF(is_eligible_open_pipeline = TRUE AND base.is_stage_1_plus = TRUE, calculated_deal_count, 0)   AS open_1plus_deal_count,
    IFF(is_eligible_open_pipeline = TRUE AND base.is_stage_3_plus = TRUE, calculated_deal_count, 0)   AS open_3plus_deal_count,
    IFF(is_eligible_open_pipeline = TRUE AND base.is_stage_4_plus = TRUE, calculated_deal_count, 0)   AS open_4plus_deal_count,
    IFF(is_booked_net_arr = TRUE, calculated_deal_count, 0)                                           AS booked_deal_count,
    IFF(is_eligible_churn_contraction = TRUE, calculated_deal_count, 0)                               AS churned_contraction_deal_count,
    CASE 
      WHEN (
            (base.is_renewal = TRUE AND is_lost = TRUE)
            OR sfdc_opportunity_stage.is_won = TRUE
          )
          AND is_eligible_churn_contraction = TRUE 
    ...
    IFF(is_eligible_churn_contraction = TRUE, net_arr, 0)                                             AS churned_contraction_net_arr,
    IFF(is_eligible_open_pipeline = TRUE, net_arr, 0)                                                 AS open_1plus_net_arr,
    IFF(is_eligible_open_pipeline = TRUE AND base.is_stage_3_plus = TRUE, net_arr, 0)                 AS open_3plus_net_arr,
    IFF(is_eligible_open_pipeline = TRUE AND base.is_stage_4_plus = TRUE, net_arr, 0)                 AS open_4plus_net_arr,
    IFF(is_booked_net_arr = TRUE, net_arr, 0)                                                         AS booked_net_arr,
    CASE
      WHEN base.deal_path_live = 'Partner'
    ...
    CASE
      ...
      WHEN base.is_jihu_account_live = TRUE
        THEN TRUE
      -- remove deleted opps
      WHEN base.is_deleted = TRUE
        THEN TRUE
      ELSE FALSE
    ...
    CASE
      ...
      WHEN base.is_open = FALSE AND base.snapshot_date < base.close_date
        THEN DATEDIFF(DAYS, base.created_date, base.snapshot_date)
    ...
    -- The following calculation involving COALESCE() is intended to temporarily reconcile BASE Opportunities that have incorrect hierarchy SKs 
    -- until we update the logic of dim_crm_opp_owner_stamped_hierarchy_sk to use stamped values instead of live ones.
    COALESCE(
      base_hierarchy_sk.dim_crm_current_account_set_hierarchy_sk,
      UPPER(
        CASE 
    ...
    CASE
      WHEN base.is_renewal = TRUE
        AND is_eligible_age_analysis = TRUE
        THEN DATEDIFF(DAY, arr_created_date, close_date.date_actual)
      WHEN base.is_renewal = FALSE
        AND is_eligible_age_analysis = TRUE
  ...
  -- This code calculates sales metrics for each snapshot quarter
    ...
    CASE
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
        AND base.is_renewal = TRUE
        AND is_eligible_age_analysis = TRUE
        THEN DATEDIFF(DAY, arr_created_date, close_date.date_actual)
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
        AND base.is_renewal = FALSE
        AND is_eligible_age_analysis = TRUE
        THEN DATEDIFF(DAY, base.created_date, close_date.date_actual)
    ...
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date 
          AND is_eligible_open_pipeline = TRUE 
      THEN net_arr 
      ELSE NULL 
    ...
    CASE 
      ...
      ELSE NULL 
    END                                                                                                  AS booked_ps_value_in_snapshot_quarter,


    -- FY26 First Order (New Logo) metrics. Metric names are aligned with https://internal.gitlab.com/handbook/enterprise-data/data-governance/data-catalog/gtm-operating-performance/first-orders/
    -- FO open 1+ pipeline count
    CASE 
      WHEN new_logo_count != 0 
          AND is_eligible_open_pipeline = TRUE 
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_open_1plus_saos,
    -- FO open 3+ pipeline count
    CASE 
      WHEN new_logo_count != 0 
          AND is_eligible_open_pipeline = TRUE
          AND base.is_stage_3_plus = TRUE 
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_open_3plus_saos,
    -- FO open 4+ pipeline count
    CASE 
      WHEN new_logo_count != 0 
          AND is_eligible_open_pipeline = TRUE
          AND base.is_stage_4_plus = TRUE 
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_open_4plus_saos,
    -- FO pipegen count
    CASE 
      WHEN new_logo_count != 0
          AND is_net_arr_pipeline_created = TRUE
    ...
    -- FO closed deals count
    CASE 
      WHEN new_logo_count != 0 
          AND base.is_closed = TRUE 
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_closed_deals,
    -- FO closed opps count
    CASE 
      WHEN new_logo_count != 0 
          AND is_win_rate_calc = TRUE 
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_closed_opps,
    -- FO closed won opps count
    CASE 
      WHEN new_logo_count != 0 
          AND is_closed_won = TRUE
          AND is_win_rate_calc = TRUE 
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_closed_won_opps,
    -- FO open 1+ pipeline sum
    CASE 
      WHEN new_logo_count != 0
          AND is_eligible_open_pipeline = TRUE 
      THEN net_arr 
      ELSE NULL 
    END                                                                                                  AS first_order_open_1plus_pipeline,
    -- FO open 3+ pipeline sum
    CASE 
      WHEN new_logo_count != 0
          AND is_eligible_open_pipeline = TRUE 
          AND base.is_stage_3_plus = TRUE 
      THEN net_arr 
      ELSE NULL 
    END                                                                                                  AS first_order_open_3plus_pipeline,
    -- FO open 4+ pipeline sum
    CASE 
      WHEN new_logo_count != 0
          AND is_eligible_open_pipeline = TRUE 
          AND base.is_stage_4_plus = TRUE 
      THEN net_arr 
      ELSE NULL 
    END                                                                                                  AS first_order_open_4plus_pipeline,
    -- FO pipegen sum
    CASE 
      WHEN new_logo_count != 0 
          AND is_net_arr_pipeline_created = TRUE
      THEN net_arr
      ELSE NULL 
    END                                                                                                  AS first_order_pipeline_generated,
    -- FO bookings sum
    CASE 
      WHEN new_logo_count != 0 
          AND is_booked_net_arr = TRUE
      THEN net_arr
      ELSE NULL 
    END                                                                                                  AS first_order_bookings,
    -- FO closed net arr sum
    CASE 
      WHEN new_logo_count != 0 
          AND base.is_closed = TRUE
      THEN net_arr
      ELSE NULL 
    END                                                                                                  AS first_order_closed_net_arr,

    -- First Order in snapshot quarter fields
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND new_logo_count != 0 
          AND is_eligible_open_pipeline = TRUE 
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_open_1plus_saos_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND new_logo_count != 0 
          AND is_eligible_open_pipeline = TRUE
      ...
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_open_3plus_saos_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND new_logo_count != 0 
          AND is_eligible_open_pipeline = TRUE
      ...
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_open_4plus_saos_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = arr_created_fiscal_quarter_date
          AND new_logo_count != 0
          AND is_net_arr_pipeline_created = TRUE
      THEN new_logo_count
      ELSE NULL 
    END                                                                                                  AS first_order_saos_generated_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND new_logo_count != 0 
          AND is_booked_net_arr = TRUE 
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_booked_deals_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date 
          AND new_logo_count != 0
          AND base.is_closed = TRUE
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_closed_deals_in_snapshot_quarter,    
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date 
          AND new_logo_count != 0
          AND is_win_rate_calc = TRUE
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_closed_opps_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date 
          AND new_logo_count != 0
          AND is_closed_won = TRUE
      ...
      THEN new_logo_count 
      ELSE NULL 
    END                                                                                                  AS first_order_closed_won_opps_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND new_logo_count != 0
          AND is_eligible_open_pipeline = TRUE 
      THEN net_arr 
      ELSE NULL 
    END                                                                                                  AS first_order_open_1plus_pipeline_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND new_logo_count != 0
          AND is_eligible_open_pipeline = TRUE 
      ...
      THEN net_arr 
      ELSE NULL 
    END                                                                                                  AS first_order_open_3plus_pipeline_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND new_logo_count != 0
          AND is_eligible_open_pipeline = TRUE 
      ...
      THEN net_arr 
      ELSE NULL 
    END                                                                                                  AS first_order_open_4plus_pipeline_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = arr_created_fiscal_quarter_date
          AND new_logo_count != 0 
          AND is_net_arr_pipeline_created = TRUE
      THEN net_arr
      ELSE NULL 
    END                                                                                                  AS first_order_pipeline_generated_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND new_logo_count != 0 
          AND is_booked_net_arr = TRUE
      THEN net_arr
      ELSE NULL 
    END                                                                                                  AS first_order_bookings_in_snapshot_quarter,
    CASE 
      WHEN base.snapshot_fiscal_quarter_date = close_fiscal_quarter_date
          AND new_logo_count != 0 
          AND base.is_closed = TRUE
      THEN net_arr
      ELSE NULL 
    END                                                                                                  AS first_order_closed_net_arr_in_snapshot_quarter,    

    -- Sales dev reporting fields
    CASE
      WHEN base.opportunity_category != 'Credit'
          AND base.sales_qualified_source = 'SDR Generated'
          AND is_net_arr_pipeline_created = TRUE
      THEN TRUE
      ELSE FALSE
    END                                                                                                  AS is_sales_dev_pipeline_created,
    CASE
      WHEN base.opportunity_category != 'Credit'
          AND base.sales_qualified_source = 'SDR Generated'
          AND is_booked_net_arr = TRUE
      ...
      THEN new_logo_count
      ELSE 0
    END                                                                                                  AS sales_dev_first_order_booked_deal_count,
    ...
    IFF(is_net_arr_pipeline_created = TRUE, net_arr, NULL)                                               AS created_arr,
    ...
    IFF(is_net_arr_pipeline_created = TRUE, calculated_deal_count, NULL)                                 AS created_deals,
    -- Fields to calculate average deal size. Net arr in the numerator / deal count in the denominator
    ...
    IFF(is_eligible_open_pipeline = TRUE AND net_arr > 0, 1, NULL)                                       AS positive_open_deal_count,
    IFF(is_eligible_open_pipeline = TRUE AND net_arr > 0, net_arr, NULL)                                 AS positive_open_net_arr,
    IFF(base.is_closed = TRUE, calculated_deal_count, NULL)                                              AS closed_deals,
    IFF(base.is_closed = TRUE, net_arr, NULL)                                                            AS closed_net_arr,
    IFF(is_booked_net_arr = TRUE, professional_services_value, NULL)                                     AS booked_ps_value,
  ...
  LEFT JOIN base_opportunity_hierarchy_sk_override_fy26 AS base_hierarchy_sk
    ON base.dim_crm_opportunity_id = base_hierarchy_sk.dim_crm_opportunity_id
