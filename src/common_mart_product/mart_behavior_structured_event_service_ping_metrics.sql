{{
  config(
    ...
    pre_hook = [
      "{{'USE WAREHOUSE ' ~ generate_warehouse_name('4XL') ~ ';' if not is_incremental() else 'USE WAREHOUSE ' ~ generate_warehouse_name('XL') ~ ';' }}"
    ],
    post_hook = ["{{'USE WAREHOUSE ' ~ generate_warehouse_name('XL') ~ ';' }}"]
  )
}}
